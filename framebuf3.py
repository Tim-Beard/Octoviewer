...
"""
framebuf3 v3.0
July 2024: Tim-Beard - Added a function (text_mx_my) to scale x and y by different amounts
...
class FrameBuffer(framebuf.FrameBuffer):
    ...
    def text_mx_my(self, s, x, y, mx, my, c: int = 1):
        """
        large text drawing function uses the standard framebuffer font (8x8 pixel characters)
        writes text, s,
        to co-cordinates x, y
        size multiple - can be different in x and y directions: mx, my (integer, eg: 1,2,3,4. a value of 2 produces 16x16 pixel characters)
        colour, c [optional parameter, default value c=1]
        optional parameter, r is rotation of the text: 0, 90, 180, or 270 degrees
        optional parameter, t is rotation of each character within the text: 0, 90, 180, or 270 degrees
        """
            ...
            for i in range(0, 8):
                for j in range(0, 8):
                    if letter.pixel(i, j) == 1:
                        p = x0 + i
                        q = y0 + j
                        self.fill_rect(x + p * mx, y + q * my, mx, my, colour)
            x += dx
            y += dy
